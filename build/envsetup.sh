...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/arsenals-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.arsenals.device | grep -q "$LINEAGE_BUILD"); then
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.arsenals.device | grep -q "$LINEAGE_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.arsenals.device | grep -q "$LINEAGE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
